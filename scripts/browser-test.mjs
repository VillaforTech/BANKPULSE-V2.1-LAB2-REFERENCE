...
function readRendered({ eventId, expectedText, businessText }) {
  ...
  const read = node => node && ({ text: node.innerText, value: node.firstElementChild?.textContent.trim(), revision: node.dataset.revision, quality: node.dataset.quality, eventId: node.dataset.eventId });
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      const finalFrame = await page.evaluate(async ({ expected, readSource }) => {
        ...
        return { end: performance.now(), rendered: read(expected) };
      ...
      samples.push({ i, aggregateId: data.id, eventId: data.lastEventId, expectedCount, expectedBusinessValue: businessText, rendered: finalFrame.rendered, start, end, milliseconds: end - start });
